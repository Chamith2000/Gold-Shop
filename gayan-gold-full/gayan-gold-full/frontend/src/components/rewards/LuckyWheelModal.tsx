...
export const LuckyWheelModal: React.FC<LuckyWheelModalProps> = ({
  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      setSpinStatus(data);
      setTimeLeft(Math.max(0, data.secondsRemaining || 0));
    } catch (err: any) {
      setErrorMessage(err?.message || "Unable to load the wheel status.");
    }
  };
  ...
  const segments = spinStatus?.segments?.length ? spinStatus.segments : fallbackSegments;
  ...
  const handleSpin = async () => {
    ...
    try {
      ...
      const targetOffset = 360 - (targetIndex + 0.5) * segmentAngle;

      // Add to the existing angle so every spin performs five complete rotations.
      setSpinAngle((previousAngle) => previousAngle + 360 * 5 + targetOffset);

      window.setTimeout(() => {
        setIsSpinning(false);
        setWonPoints(response.rewardPoints);
        setResultMessage(response.message);
        setTimeLeft(Math.max(0, response.secondsRemaining || 86400));
        refreshProfile();
      ...
      // Re-read the server state so a cooldown error is reflected immediately.
      fetchStatus();
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="fixed inset-0 bg-black/75 backdrop-blur-xs transition-opacity" onClick={() => !isSpinning && onClose()} />
      <div className="flex min-h-full items-center justify-center p-4 text-center">
        <div className="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-[#FAF8F5] border-2 border-[#D4AF37] p-6 shadow-2xl transition-all font-sans animate-in zoom-in-95 duration-200">
          <button onClick={() => !isSpinning && onClose()} disabled={isSpinning} className="absolute top-4 right-4 text-stone-400 hover:text-stone-700 p-1.5 rounded-full hover:bg-stone-100 transition-colors disabled:opacity-50">
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="space-y-1 mb-6">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#FAF3E8] border border-[#D4AF37]/50 text-[#B54E0E] text-xs font-bold uppercase tracking-wider">
              <Flame className="w-4 h-4 text-[#D4AF37]" /> Daily Royal Wheel of Fortune
            </div>
            <h3 className="font-cinzel text-xl sm:text-2xl font-bold text-[#5A0F1B] uppercase tracking-wider">Spin to Win Gold Points</h3>
            <p className="text-xs text-stone-600">Every patron is entitled to one complimentary spin every 24 hours. Points redeemable for instant checkout discounts!</p>
          </div>
          ...
          <div className="relative w-72 h-72 sm:w-80 sm:h-80 mx-auto my-4 flex items-center justify-center">
            ...
            >
              <svg viewBox="0 0 400 400" className="w-full h-full">
                {segments.map((seg, index) => {
                  ...
                  return (
                    <g key={seg.id || index}>
                      <path d={pathData} fill={seg.color || (index % 2 === 0 ? "#5A0F1B" : "#B54E0E")} stroke="#D4AF37" strokeWidth="2" />
                      <text x="200" y="60" transform={`rotate(${midAngle}, 200, 200)`} fill="#FAF8F5" fontSize="13" fontWeight="bold" fontFamily="Cinzel, serif" textAnchor="middle" dominantBaseline="middle">
                        {seg.name}
                      </text>
                    </g>
                  );
            ...
            <div className="absolute z-20 w-20 h-20 rounded-full bg-gradient-to-br from-[#FAF8F5] to-[#F3E5AB] border-4 border-[#D4AF37] shadow-lg flex flex-col items-center justify-center p-1">
              <span className="font-cinzel text-xs font-bold text-[#5A0F1B] leading-none">GAYAN</span>
              <span className="text-[8px] font-bold text-[#B54E0E] uppercase tracking-widest mt-0.5">GOLD</span>
          ...
          {resultMessage && (
            <div className="my-3 p-3 bg-emerald-50 border border-emerald-300 rounded-xl text-emerald-800 animate-in fade-in">
              ...
              {wonPoints !== null && wonPoints > 0 && <p className="text-xs font-semibold text-emerald-700 mt-1">+{wonPoints} Points added to your royal balance!</p>}
            </div>
          )}
          {errorMessage && (
          ...
          <div className="mt-5 space-y-3">
            {!isAuthenticated ? (
              <button onClick={() => { onClose(); onNavigateToLogin(); }} className="w-full bg-[#5A0F1B] text-white py-3 rounded-xl font-cinzel text-xs font-bold uppercase tracking-wider hover:bg-[#400A13] transition-colors shadow-md flex items-center justify-center gap-2">Sign In to Spin the Royal Wheel</button>
            ) : timeLeft > 0 ? (
              <div className="bg-stone-100 border border-stone-200 rounded-xl py-3 px-4 flex items-center justify-center gap-2 text-stone-600">
                <Clock className="w-4 h-4 text-[#B54E0E]" /><span className="text-xs font-medium">Next Spin Available in:</span><span className="font-mono text-xs font-bold text-[#5A0F1B]">{formatCountdown(timeLeft)}</span>
              </div>
            ) : (
              <button onClick={handleSpin} disabled={isSpinning} className="w-full bg-gradient-to-r from-[#5A0F1B] via-[#8A1C2C] to-[#B54E0E] hover:opacity-95 text-[#FAF8F5] py-3.5 rounded-xl font-cinzel text-sm font-bold uppercase tracking-wider transition-all shadow-lg border border-[#D4AF37]/50 disabled:opacity-50 flex items-center justify-center gap-2" id="lucky-spin-action-btn">
                <Flame className="w-4 h-4 text-[#D4AF37]" />{isSpinning ? "Spinning Royal Wheel..." : "Spin the Wheel Now"}
              </button>
            )}
            <div className="flex items-center justify-center gap-4 text-[11px] text-stone-500 pt-1">
              <span className="flex items-center gap-1"><Award className="w-3.5 h-3.5 text-[#D4AF37]" /> 100% Free Daily</span><span>•</span><span>1 Pt = Rs. 1.00 Off</span><span>•</span><span>Tier Multipliers Apply</span>
            </div>
          </div>
        </div>
      </div>
